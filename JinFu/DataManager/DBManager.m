 ...
 */
NSString *billInfoDataTable = @"create table IF NOT EXISTS 'BillInfoDataTable' (period text, integral text, newlyGainedIntegral text, debt text, repayment text, billId text, cardId text, billMonth text, id integer primary key autoincrement)";
/*
...
*/
NSString *withdrawDetailTable = @"create table IF NOT EXISTS 'WithdrawDetailTable' (date text, count text, state text, id integer primary key autoincrement)";
...
- (NSMutableArray *)queryBankCardListData{
    ...
    while ([result next]) {
        ...
        cardData.address = [result stringForColumn:@"address"];
...
//
    ...
    for (BillInfoDataModel *billModel in billArr) {
        NSString *sqlstr = [NSString stringWithFormat:@"INSERT INTO 'BillInfoDataTable' (period, integral, newlyGainedIntegral, debt, repayment, billId, cardId, billMonth ) VALUES ('%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@') ", billModel.period, billModel.integral, billModel.newlyGainedIntegral, billModel.debt, billModel.repayment , billModel.billId, billModel.cardId, billModel.billMonth];
...
- (NSMutableArray *)queryBillInfoDataWithCardId:(NSString *)cardId {
    ...
    while ([result next]) {
        ...
        billdData.billMonth = [result stringForColumn:@"billMonth"];
...
- (void)saveWithdrawDetail:(NSArray *)withDrawData {
    ...
    for (WithdrawDetail *withDrawModel in withDrawData) {
        NSString *sqlstr = [NSString stringWithFormat:@"INSERT INTO 'WithdrawDetailTable' (date, count, state) VALUES ('%@', '%@', '%@') ", withDrawModel.date, withDrawModel.count, withDrawModel.state];
        DLog(@"sqlstr = %@", sqlstr);
        [db executeUpdate:sqlstr];
    }
    [db close];
...
- (NSMutableArray *)queryWithdrawDetail {
    ...
    while ([result next]) {
        ...
        withdrawModel.state = [result stringForColumn:@"state"];
