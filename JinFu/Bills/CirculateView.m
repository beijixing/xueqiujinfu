...
-(void)reloadDataWithArray:(NSArray *)array
{
    ...
    for(AdDataModel *adModel in _dataArray)
    {
        ...
        if ([adModel.imageUrlStr hasPrefix:@"http://"]) {
            [imageView sd_setImageWithURL: [NSURL URLWithString: adModel.imageUrlStr] placeholderImage:[UIImage imageNamed:@"beijingtu"]];
        }else {
            if (adModel.imageUrlStr == nil || adModel.imageUrlStr.length == 0) {
                imageView.image = [UIImage imageNamed:@"beijingtu"];
            }else {
