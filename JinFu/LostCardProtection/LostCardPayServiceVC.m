...
@interface LostCardPayServiceVC ()
{
    NSArray *_payWays;
...
@implementation LostCardPayServiceVC
...
- (void)viewDidLoad {
    ...
    self.lostCardinfoTable.separatorStyle = UITableViewCellSeparatorStyleNone;
...
- (ProtectionTitleCell *)getProtectionTitleCellOfTableView:(UITableView *)tableView andIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.image.image = ImageName(@"diu-ka-bao-zhang");
    cell.title.text = @"丢卡保障";
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section ==1) {
        if (indexPath.row == 0) {
            DLog(@"微信支付");
            [[WXApiManager sharedManager] payOrderWithCost:self.unautoProtectionInfo.totalCost subject:self.unautoProtectionInfo.name];
        }else if(indexPath.row == 1)
        {
            DLog(@"支付宝支付");
            [[AliPayApiManager sharedManager] payOrderWithCost:self.unautoProtectionInfo.totalCost subject:self.unautoProtectionInfo.name];
        }
    }
