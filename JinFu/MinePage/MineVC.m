...
- (void)viewDidLoad {
    ...
    if ([AFNetworkReachabilityManager sharedManager].isReachable == 0) {
        ...
        NSString * URL = [NSString stringWithFormat:@"%@%@", HostUrl, LoginMemberUsername];
...
- (void)initCellData {
    ...
    NSArray *peifu = [NSArray arrayWithObjects:@"compensate", @"交易记录", nil];
    [cellDataArr addObject:peifu];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        CompensateVC *compensateVc = [[CompensateVC alloc] init];
        [self.navigationController pushViewController:compensateVc animated:YES];
    }
}
