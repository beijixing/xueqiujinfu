...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        BillDetailDataModel *billDetailData = [details objectAtIndex:indexPath.row - 2];
...
//    DLog(@"period = %@",[dataDict objectForKey:@"period"]);
    ...
    NSString *period = billInfoModel.period;
//    DLog(@"length = %d", [period length]);
    headerFooterView.month = [NSString stringWithFormat:@"%@月", [period substringWithRange:NSMakeRange(18, 2)]];
    headerFooterView.isOpen = [_sourceDataOpenState[section] boolValue];
    [headerFooterView.contentView setMyViewLayerBoardColor:[UIColor yellowColor] AndBoardWidth:1 AndCornerRadius:10];
